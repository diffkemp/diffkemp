...
bool DifferentialFunctionComparator::maySkipCast(const User *Cast) const {
    ...
    if (auto StrTy = dyn_cast<StructType>(SrcTy)) {
        if (StrTy->hasName() && hasPrefix(StrTy->getName(), "union")) {
...
bool DifferentialFunctionComparator::cmpCallArgumentUsingCSource(
    ...
    if (hasPrefix(CFL->getName(), SimpllInlineAsmPrefix))
        CArgsL =
    ...
    if (hasPrefix(CFR->getName(), SimpllInlineAsmPrefix))
        CArgsR =
    ...
    if ((CArgsL.size() > i) && (CArgsR.size() > i)) {
        ...
        if (hasPrefix(StringRef(CArgsL[i]), "sizeof")
            && hasPrefix(StringRef(CArgsR[i]), "sizeof")
...
std::vector<std::unique_ptr<SyntaxDifference>>
    ...
    if (!hasPrefix(FunL->getName(), SimpllInlineAsmPrefix)
        || !hasPrefix(FunR->getName(), SimpllInlineAsmPrefix))
...
void DifferentialFunctionComparator::findTypeDifference(
    ...
    if (cmpTypes(L, R)) {
        ...
        diff->name = hasPrefix(L->getName(), "struct.")
...
bool DifferentialFunctionComparator::isUnionTy(const Type *Ty) {
    if (auto StrTy = dyn_cast<StructType>(Ty)) {
        return hasPrefix(StrTy->getStructName(), "union");
    }
    return false;
}
