...
class GlobalNumberState {
  ...
  public:
    ...
    virtual uint64_t getNumber(GlobalValue *Global) {
        ...
        bool Inserted;
        std::tie(MapIter, Inserted) =
                GlobalNumbers.insert({Global, NextNumber});
        if (Inserted)
            NextNumber++;
        return MapIter->second;
    }

    virtual void erase(GlobalValue *Global) { GlobalNumbers.erase(Global); }

    virtual void clear() { GlobalNumbers.clear(); }
};
...
class FunctionComparator {
  public:
    FunctionComparator(const Function *F1,
                       const Function *F2,
    ...
    /// unequal functions will have different hashes with high probability.
  ...
  protected:
    ...
    /// Compares the signature and other general attributes of the two
    /// functions.
    virtual int compareSignature() const;

    /// Test whether two basic blocks have equivalent behaviour.
    virtual int cmpBasicBlocks(const BasicBlock *BBL,
    ...
    /// Stage 0: If types are equal in terms of cmpTypes, then we can go
    /// straight
    ...
    ///          are bitcastable, and we can skip other stages, and go to
    ///          contents comparison.
    ...
    /// 3. And again about overall picture. Let's look back at how the ordered
    /// set of constants will look like: [NFCT], [FCT, "others"], [FCT,
    /// pointers], [FCT, vectors]
    ///
    ...
    /// The structures. Structures are grouped at the tail, again because of
    /// their TypeID: StructTyID > IntegerTyID > FloatTyID. Structures with
    /// greater number of elements are greater. Structures with greater elements
    /// going first are greater. The same logic with vectors, arrays and other
    ...
    ///    (if TyA is pointer), or else (if TyA equal to TyB), those types
    ///    should be equal to TyB.
    ...
    /// 2.2. All pointers of the same address space, no matter what they point
    /// to,
    ///      bitcastable. So if C is pointer, it could be bitcasted to A and to
    ///      B.
    ...
    ///          If left and right values are references to their functions,
    ///          then they are equal.
    ...
    virtual int cmpOperations(const Instruction *L,
                              const Instruction *R,
    ...
    /// be checked with the same way. If we get Res != 0 on some stage, return
    ...
    /// 2. Constant offset, (using GEPOperator::accumulateConstantOffset
    /// method).
    ...
    /// In this case it would be good to explain few points about values
    /// assigned to BBs and other ways of implementation (see below).
    ///
    ...
    /// from the entry, and then take each terminator. So it doesn't matter how
    /// in fact BBs are ordered in function. And since cmpValues are called
    /// during this walk, the numbering depends only on how BBs located inside
    /// the CFG. So the answer is - yes. We will get the same numbering.
    ///
