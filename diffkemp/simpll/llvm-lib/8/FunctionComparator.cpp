...
int FunctionComparator::cmpOperations(const Instruction *L,
    ...
    if (const AllocaInst *AI = dyn_cast<AllocaInst>(L)) {
        ...
        return cmpNumbers(AI->getAlignment(),
                          cast<AllocaInst>(R)->getAlignment());
    }
    if (const LoadInst *LI = dyn_cast<LoadInst>(L)) {
        if (int Res = cmpNumbers(LI->isVolatile(),
                                 cast<LoadInst>(R)->isVolatile()))
            return Res;
        if (int Res = cmpNumbers(LI->getAlignment(),
                                 cast<LoadInst>(R)->getAlignment()))
            return Res;
    ...
    if (const StoreInst *SI = dyn_cast<StoreInst>(L)) {
        if (int Res = cmpNumbers(SI->isVolatile(),
            ...
            return Res;
        if (int Res = cmpNumbers(SI->getAlignment(),
                                 cast<StoreInst>(R)->getAlignment()))
            return Res;
    ...
    if (const CallInst *CI = dyn_cast<CallInst>(L)) {
        if (int Res = cmpNumbers(CI->getCallingConv(),
            ...
            return Res;
        if (int Res = cmpAttrs(CI->getAttributes(),
                               cast<CallInst>(R)->getAttributes()))
            return Res;
    ...
    if (const InvokeInst *II = dyn_cast<InvokeInst>(L)) {
        if (int Res = cmpNumbers(II->getCallingConv(),
            ...
            return Res;
        if (int Res = cmpAttrs(II->getAttributes(),
                               cast<InvokeInst>(R)->getAttributes()))
            return Res;
...
FunctionComparator::FunctionHash FunctionComparator::functionHash(Function &F) {
    ...
    while (!BBs.empty()) {
        ...
        // of opcodes into BBs wouldn't affect the hash, only the order of the
        // opcodes
        H.add(45798);
        for (auto &Inst : *BB) {
