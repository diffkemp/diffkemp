...
int FunctionComparator::cmpInstMetadata(Instruction const *L,
    ...
    /// These metadata affects the other optimization passes by making
    /// assertions or constraints. Values that carry different expectations
...
int FunctionComparator::cmpOperandBundlesSchema(const CallBase &LCS,
    ...
    if (int Res = cmpNumbers(LCS.getNumOperandBundles(),
                             RCS.getNumOperandBundles()))
        return Res;
...
int FunctionComparator::cmpConstants(const Constant *L,
    ...
    if (TypesRes != 0) {
        ...
        if (!TyL->isFirstClassType()) {
            if (TyR->isFirstClassType())
                return -1;
            // Neither TyL nor TyR are values of first class type. Return the
            // result of comparing the types
            return TypesRes;
        }
        if (!TyR->isFirstClassType()) {
            if (TyL->isFirstClassType())
        ...
        // types have the same size, otherwise not.
        unsigned TyLWidth = 0;
        unsigned TyRWidth = 0;
    ...
    if (const auto *SeqL = dyn_cast<ConstantDataSequential>(L)) {
        ...
        // host endianness. This isn't a problem though, because the endiness of
        // a module will affect the order of the constants, but this order is
        // the same for a given input module and host platform.
    ...
    case Value::ConstantExprVal: {
        ...
        if (auto *GEPL = dyn_cast<GEPOperator>(LE)) {
            ...
            if (int Res = cmpNumbers(GEPL->isInBounds(), GEPR->isInBounds()))
                return Res;
            if (int Res = cmpNumbers(
                        GEPL->getInRangeIndex().value_or(unsigned(-1)),
        ...
        if (auto *OBOL = dyn_cast<OverflowingBinaryOperator>(LE)) {
            ...
            if (int Res = cmpNumbers(OBOL->hasNoUnsignedWrap(),
                                     OBOR->hasNoUnsignedWrap()))
                return Res;
            if (int Res = cmpNumbers(OBOL->hasNoSignedWrap(),
                                     OBOR->hasNoSignedWrap()))
                return Res;
        }
        return 0;
    }
    case Value::BlockAddressVal: {
        ...
        if (LBA->getFunction() == RBA->getFunction()) {
            // They are BBs in the same function. Order by which comes first in
            // the BB order of the function. This order is deterministic.
            ...
            // literally the same pointer, they must respectively be the left
            // and right functions.
            assert(LBA->getFunction() == FnL && RBA->getFunction() == FnR);
            // cmpValues will tell us if these are equivalent BasicBlocks, in
            // the context of their respective functions.
            return cmpValues(LBA->getBasicBlock(), RBA->getBasicBlock());
        }
    }
    case Value::DSOLocalEquivalentVal: {
        // dso_local_equivalent is functionally equivalent to whatever it points
        // to. This means the behavior of the IR should be the exact same as if
        // the function was referenced directly rather than through a
        // dso_local_equivalent.
        const auto *LEquiv = cast<DSOLocalEquivalent>(L);
        const auto *REquiv = cast<DSOLocalEquivalent>(R);
        return cmpGlobalValues(LEquiv->getGlobalValue(),
                               REquiv->getGlobalValue());
    }
...
int FunctionComparator::cmpTypes(Type *TyL, Type *TyR) const {
    ...
    case Type::StructTyID: {
        ...
        for (unsigned i = 0, e = STyL->getNumElements(); i != e; ++i) {
            if (int Res = cmpTypes(STyL->getElementType(i),
    ...
    case Type::FunctionTyID: {
        ...
        for (unsigned i = 0, e = FTyL->getNumParams(); i != e; ++i) {
            if (int Res =
                        cmpTypes(FTyL->getParamType(i), FTyR->getParamType(i)))
                return Res;
        }
        return 0;
    }
...
int FunctionComparator::cmpOperations(const Instruction *L,
    ...
    //  * because of the above, we don't test for the tail bit on calls later
    //  on.
    ...
    if (const GetElementPtrInst *GEPL = dyn_cast<GetElementPtrInst>(L)) {
        ...
        if (int Res = cmpValues(GEPL->getPointerOperand(),
                                GEPR->getPointerOperand()))
            return Res;
    ...
    if (const FenceInst *FI = dyn_cast<FenceInst>(L)) {
        if (int Res = cmpOrderings(FI->getOrdering(),
                                   cast<FenceInst>(R)->getOrdering()))
            return Res;
    ...
    if (const PHINode *PNL = dyn_cast<PHINode>(L)) {
        ...
        for (unsigned i = 0, e = PNL->getNumIncomingValues(); i != e; ++i) {
            if (int Res = cmpValues(PNL->getIncomingBlock(i),
                                    PNR->getIncomingBlock(i)))
                return Res;
        }
    }
    return 0;
