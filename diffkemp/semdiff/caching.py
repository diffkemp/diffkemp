...
class ComparisonGraph:
    ...
    class Vertex:
        ...
        Note: names, glob_vars, files and lines are tuples containing the
        values for both modules.
        """
        def __init__(self, names, result, glob_vars=None, files=None,
        ...
        def __repr__(self):
            return "Vertex({0}, {1}, {2}, {3}, {4}, {5})".format(
                self.names, self.result, self.glob_vars, self.nonfun_diffs,
                self.files, self.lines
            )

        def dict_key(self, side):
            return self.names[side], self.glob_vars[side]
    ...
    class Edge:
        ...
        """
        def __init__(self, target_name, target_glob_var, filename, line):
            self.parent_vertex = None
            self.target_name = target_name
            self.target_glob_var = target_glob_var
            self.filename = filename
            self.line = line
            self.kind = ComparisonGraph.DependencyKind.STRONG

        def __repr__(self):
            return "Edge({0}, {1}, {2}, {3})".format(
                self.target_name, self.target_glob_var, self.filename,
                self.line
            )

        def dict_key(self):
            return self.target_name, self.target_glob_var

        @classmethod
        def from_yaml(cls, callee, glob_var):
            """Generates an Edge object from YAML returned by SimpLL."""
            res = cls(callee["function"], glob_var, callee["file"],
                      int(callee["line"]))
    ...
    def __init__(self):
        # Vertices are stored as a dictionary, the keys being tuples
        # (function_name, global_variable) in the left module and the
        # values objects containing the result
        self.vertices = dict()
        # set containing tuples (func_name, glob_var)
        self.equal_funs = set()
    ...
    def __getitem__(self, vertex_key):
        if isinstance(vertex_key, str):
            vertex_key = vertex_key, None
        return self.vertices[vertex_key]

    def __setitem__(self, vertex_key, value):
        if isinstance(vertex_key, str):
            vertex_key = vertex_key, None
        self.vertices[vertex_key] = value
        if value.result == Result.Kind.EQUAL:
            self.equal_funs.add(value.dict_key(ComparisonGraph.Side.LEFT))
        if vertex_key[0].endswith(".void"):
    ...
    def reachable_from(self, side, start_fun_name, glob_var=None):
        """
        ...
        the function name and the global variable including the base vertex).
        ...
        """
        start_fun = self[(start_fun_name, glob_var)]
        ...
        while queue:
            ...
            for edge in current.successors[side]:
                if edge.dict_key() not in self.vertices:
                    # Invalid successor.
                    continue
                target = self[edge.dict_key()]
    ...
    def normalize(self):
                ...
                if edge.parent_vertex:
                    ...
                    for side in ComparisonGraph.Side:
                        for e in edge.parent_vertex.successors[side]:
                            if e.dict_key() == (unpointed_name,
                                                edge.target_glob_var):
        ...
        for weak_vertex in self._weak_vertex_cache:
            ...
            glob_var = (weak_vertex.glob_vars[ComparisonGraph.Side.LEFT] if
                        weak_vertex.names[ComparisonGraph.Side.LEFT].
                        endswith(".void")
                        else weak_vertex.glob_vars[ComparisonGraph.Side.RIGHT])
            non_pointed_name = pointed_name[:-len(".void")]
            del self.vertices[(pointed_name, glob_var)]
            if (non_pointed_name, glob_var) not in self.vertices:
                ...
                self[(non_pointed_name, glob_var)] = weak_vertex
    ...
    def graph_to_fun_pair_list(self, fun_first, fun_second, full_diff,
                               glob_var_first=None, glob_var_second=None):
        ...
        called_funs_left, backtracking_map_left = self.reachable_from(
            ComparisonGraph.Side.LEFT, fun_first, glob_var_first)
        called_funs_right, backtracking_map_right = self.reachable_from(
            ComparisonGraph.Side.RIGHT, fun_second, glob_var_second)
        ...
        for vertex in vertices_to_compare:
            ...
            for nonfun_diff in vertex.nonfun_diffs:
                ...
                for side in ComparisonGraph.Side:
                    ...
                    glob = glob_var_first if \
                        side == ComparisonGraph.Side.LEFT else glob_var_second
                    if nonfun_diff.parent_fun != fun:
                        parent_calls = Result.Callstack.from_edge_objects(
                            get_callstack(backtracking_map, self[(fun, glob)],
                                          vertex))
