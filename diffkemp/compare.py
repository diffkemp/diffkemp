...
def compare(args):
    ...
    # After compare finishes, displays: - output_dir if specified
    #                                   - stats if flag was set
    _final_output(args, output_dir, result)
    return 0
...
def _set_output_dir(compare_args):
    ...
    if os.path.isdir(output_dir):
        raise OutputDirExistsError("Error: output directory {}\
                                    exists\n".format(output_dir))
    return output_dir
...
def _print_fun_result(fun_result, config, output_dir, fun, group_dir,
    ...
    if not (fun_result.kind == Result.Kind.NOT_EQUAL or
       config.full_diff):
...
def _create_yaml_output(args, result, output_dir):
...
def _final_output(args, output_dir, result):
    if output_dir is not None and os.path.isdir(output_dir):
