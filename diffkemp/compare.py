...
class SnapshotComparator:
    ...
    def _compare_snapshots(self):
        for group_name, group in sorted(self.config.snapshot_first
                                        .fun_groups.items()):
            self._set_group_variables(group, group_name)
            self._compare_groups()

        self._finalize_output()
        return 0

    def _set_group_variables(self, group, group_name):
        self.group = group
        self.group_name = group_name
        self._set_group_dir()
        self.group_printed = False
        self._set_modules_to_cache_if_enabled()
        self.result_graph = None
        self.cache = SimpLLCache(mkdtemp())
        self.module_cache = {}

    def _set_group_dir(self):
        if self.output_dir is not None and self.group_name is not None:
            self.group_dir = os.path.join(self.output_dir, self.group_name)
        else:
            self.group_dir = None

    def _set_modules_to_cache_if_enabled(self):
        if self.args.enable_module_cache:
            self.modules_to_cache = self._get_modules_to_cache()
        else:
            self.modules_to_cache = set()
    ...
    def print_syntax_diff(self, fun, fun_result, fun_tag):
        """
        ...
        """
        output_dir = self.group_dir if self.group_dir else self.output_dir
